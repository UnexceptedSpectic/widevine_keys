...
def load_headers():
    HEADERS_FILE = environ.get("HEADERS_FILE")
    headers_module_name = Path(HEADERS_FILE).stem
    spec = importlib.util.spec_from_file_location(headers_module_name, HEADERS_FILE)
    headers = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(headers)
    return headers


def WV_Function(pssh, lic_url, headers, only_basic: bool = True, cert_b64=None):
    ...
    # basic, most sites works
    responses.append(
        requests.post(
            url=lic_url, headers=headers.headers, data=raw_request, params=params
        )
    )
    if not only_basic:
...
def run(pssh: str, lic_url: str) -> list[str]:
    headers = load_headers()
    _, keys = WV_Function(pssh, lic_url, headers)
    return keys
