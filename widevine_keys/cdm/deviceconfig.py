...
DEVICES_FOLDER = os.environ.get("DEVICES_FOLDER")
if DEVICES_FOLDER is None:
    raise Exception("The environment variable DEVICES_FOLDER is not defined.")
    ...
    def __init__(self, device):
        ...
        if "keybox_filename" in device:
            self.keybox_filename = os.path.join(
                DEVICES_FOLDER, device["name"], device["keybox_filename"]
            )
        else:
            self.keybox_filename = os.path.join(
                DEVICES_FOLDER, device["name"], "keybox"
            )

        if "device_cert_filename" in device:
            self.device_cert_filename = os.path.join(
                DEVICES_FOLDER, device["name"], device["device_cert_filename"]
            )
        else:
            self.device_cert_filename = os.path.join(
                DEVICES_FOLDER, device["name"], "device_cert"
            )

        if "device_private_key_filename" in device:
            self.device_private_key_filename = os.path.join(
                DEVICES_FOLDER, device["name"], device["device_private_key_filename"]
            )
        else:
            self.device_private_key_filename = os.path.join(
                DEVICES_FOLDER, device["name"], "device_private_key"
            )

        if "device_client_id_blob_filename" in device:
            self.device_client_id_blob_filename = os.path.join(
                DEVICES_FOLDER, device["name"], device["device_client_id_blob_filename"]
            )
        else:
            self.device_client_id_blob_filename = os.path.join(
                DEVICES_FOLDER, device["name"], "device_client_id_blob"
            )

        if "device_vmp_blob_filename" in device:
            self.device_vmp_blob_filename = os.path.join(
                DEVICES_FOLDER, device["name"], device["device_vmp_blob_filename"]
            )
        else:
            self.device_vmp_blob_filename = os.path.join(
                DEVICES_FOLDER, device["name"], "device_vmp_blob"
            )
